...
export async function handleAddCommand(components, { force }) {
	...
	if (addedComponents.length > 0) {
		...
		for (const addedComponent of addedComponents.toReversed()) {
			if (force || !addedComponent.alreadyAdded) {
				createComponent(addedComponent.content, addedComponent.fullPath);
			}

			logResult(
				addedComponent.component,
				addedComponent.indentLevel + 1,
				addedComponent.alreadyAdded,
			);
		}
	}
}
