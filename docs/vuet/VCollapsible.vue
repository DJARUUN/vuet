...
<template>
  ...
  >
    <VButton
      ...
      :class="
        twMerge(
          `text-[0.925rem] justify-between ${isCollapsed && 'text-muted-fg hover:text-fg'}`,
    ...
    <div
      :class="
        twMerge(
          `grid transition-[grid-template-rows,opacity] ease-smooth ${isCollapsed ? 'grid-rows-[0fr] opacity-75' : 'grid-rows-[1fr]'}`,
          variantStyles[variant].inner,
        )
      "
    >
      <div
