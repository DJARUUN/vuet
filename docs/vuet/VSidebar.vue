...
<template>
  <aside class="hidden lg:flex flex-col gap-5 p-6 border-r border-border min-w-[18rem] overflow-auto">
  ...
  <nav class="hidden not-lg:flex flex-col gap-5 px-6 py-4 border-b border-border min-w-[18rem] overflow-auto">
  ...
  <Transition>
    <div v-show="isMobileNavShown"
      class="lg:hidden block fixed z-[100] top-16 bg-bg p-6 bottom-0 inset-x-0 pt-0 overflow-auto">
      <div class="flex flex-col gap-5 transition-[translate,opacity] origin-top ease-bounce duration-250">
        <template v-for="item in items" :key="item">
          <VSidebarGroup v-if="item.page === false" :item="item" />
          <VSidebarItem v-else :item="item" />
        </template>
